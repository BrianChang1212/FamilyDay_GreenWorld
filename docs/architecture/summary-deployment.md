...
與下方「全端／API 上線」不同：僅將 **Vite 建置產物**（`familyday-frontend/dist`）放於 **Netlify**、**GitHub Pages** 等靜態主機，供手機試操作。倉庫內含 **`netlify.toml`**、**`.github/workflows/deploy-github-pages.yml`**；**例行 CI** 另見 **`.github/workflows/ci.yml`**（`npm run test` → `npm run build`）。**測試 Web UI** 完整步驟（Netlify／GitHub Pages、QR、區網試機）見 [`static-preview-netlify-github.md`](../setup/static-preview-netlify-github.md)；**短摘要與錨點**見 [`overview/root-readme-supplement.md#preview-netlify-test-ui`](../overview/root-readme-supplement.md#preview-netlify-test-ui)。**未接 API** 時，領取成功頁以前端 **`sessionStorage`** 類比並標示預覽用（見 [`summary-frontend.md`](./summary-frontend.md) §2.1 **`local-fallback`**）。
...
| 1.3 | 2026-04-18 | **§1.1**：「測試 Web UI 操作」段 **`README`** 改為可點連結 [`root-readme-supplement#preview-netlify-test-ui`](../overview/root-readme-supplement.md#preview-netlify-test-ui)（長文已遷至該檔；**連結 2026-05-11 修正**） |
