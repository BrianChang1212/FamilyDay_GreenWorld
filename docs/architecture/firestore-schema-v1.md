...
實作參考：`familyday-backend/src/state/roster.ts`、`checkins.ts`、`game.ts`、`redeem.ts`。
...
| 項目 | 倉庫檔案 | 說明 |
|------|----------|------|
| Security Rules | `familyday-backend/firestore.rules` | 目前為 **全面關閉直接讀寫**（僅 API + Admin SDK 路徑）；若未來前端改用 Client SDK 讀 Firestore，再分集合放寬。 |
| 複合索引 | `familyday-backend/firestore.indexes.json` | 目前為空陣列；若查詢出現「需要建立索引」連結，再把索引條目加進此檔後部署。 |
| Firebase 設定 | `familyday-backend/firebase.json` | 已掛上 `firestore.rules` 與 `firestore.indexes.json`。 |

**部署指令**（在 `familyday-backend/`、已 `firebase login` 且 `familyday-backend/.firebaserc` 指向目標專案）：
...
> 與 Console 截圖一致：目標專案（見 `fdgw.project.json` 之 `firebaseProjectId`）→ `checkins` → 文件（例：**`1141043`**）內含 `checkinAt`、`employeeId`、`name`、`partySize`。  
...
程式型別：`familyday-backend/src/state/checkins.ts` 之 `CheckinRecord`。
...
文件 ID = **`employeeId`**。欄位與 `familyday-backend/src/state/game.ts` 之 `PlayerProgress` 一致（文件中**不含** `eventId`／`updatedAt`／`allCompleted`／`canStartNewRound`；後兩者由 API 組裝）。
...
- ~~`redeemSeq` 與 `rewardRedeemCount`~~：`confirmRedeem` 已以 Firestore transaction 同步寫入 `redeem_records` 並遞增 `rewardRedeemCount`（見 `familyday-backend/src/state/redeem.ts`）。
