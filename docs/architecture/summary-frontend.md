...
| `/scan?t=<JWT>` / `/scan?stage=<N>` | **外部 QR scanner 進入點**（活動正式 PNG 內含 `https://<host>/scan?t=<JWT>`）。`router/index.ts` redirect：解析 stage → 寫 `entryIntent="game"`、`fdgw_stage`、`fdgw_inZone=1`、`fdgw_pending_station_challenge` → 已登入直接 **`/quiz?challengeId=cN`**；未登入導 **`/register`**（登入完成讀 pending 接續至 quiz）。解析失敗 fallback 至 **`/stage`** |
| 任意路徑 `?entry=checkin`／`?entry=game` | `beforeEach` 寫入意圖（`lib/entryIntent.ts`）；仍須自行進入 **`/check-in`** 或 **`/game`** 等才觸發 redirect |
| `/register` | **闖關登入頁**（`RegisterView`）：姓名、員編；`game` 意圖時標題「請輸入您的基本資料」。若誤入且意圖為 `checkin` → **`replace`** 路由 **`checkinWelcome`**（路徑 **`/checkin`**）。送出後：(a) `fdgw_pending_station_challenge` 有值（自 `/scan` 進來）→ **`/quiz?challengeId=cN`**；(b) `game` 或無意圖（已看過說明）→ **`stage`**。**登入後僅 `syncLocalProgressFromDashboard()` 還原 `completedStageIds`，不呼叫 `restartPlaythrough`**（避免全破玩家紀錄被洗掉） |
