...
- Docs (UI assets, no contract change): StageView 闖關進度 header swaps the bar-chart inline SVG for a mountain-with-flag silhouette. FinishView 領獎狀態 header swaps the trophy outline for a medal+star+ribbon silhouette. FinishView reward slots drop the inline gift-box SVG (plus orange/dashed wrapper) and use designer-prepared PNGs `/images/Icon_gift_s_active.png` (claimed) and `/images/Icon_gift_s_disabled.png` (pending). **No REST contract change.**
- Docs (frontend behavior, no contract change): WelcomeView background reverted from `/images/01_welcomeBG.jpg` back to `/images/game-welcome-enroll-04.jpg` (commit `6ce9b52`). App.vue `route.meta.fullBleed` handling kept; BriefingView `/images/02_mapIllu.png` kept. **No REST contract change.**
- Docs (frontend behavior, no contract change): App.vue honours `route.meta.fullBleed` to drop the 90vw white card wrapper + safe-area padding on hero views (WelcomeView, CheckInWelcomeView), letting the background illustration render edge-to-edge. WelcomeView background swapped to `/images/01_welcomeBG.jpg` (1152×2048 9:16 crop of the new 2048×2048 designer asset); `familyday-logotype.png` kept as the top badge. BriefingView map swapped to `/images/02_mapIllu.png` (688×752, new flat-art). **No REST contract change.**
- Docs (frontend behavior, no contract change): FinishView "領取闖關禮" now opens a full-screen QR scanner instead of a confirm modal. `POST /api/v1/me/reward/claim` fires only after the player scans the staff-held QR encoding `fdgw-claim-token` (`familyday-frontend/public/qr-staff-stations/claim-token.png`). Payload validation lives in pure helper `familyday-frontend/src/lib/claimPayload.ts::isClaimToken`. Backend `maxRounds=3` continues to enforce the hard ceiling. **No REST contract change.**
