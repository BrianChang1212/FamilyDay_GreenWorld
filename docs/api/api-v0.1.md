...
- CORS allowlist 已收斂：**執行時**以 backend repo `fdgw.project.json` 的 **`corsOrigins`** 為準（`familyday-backend` 於載入時讀取）；下列為常見值，與該陣列一致：`http://localhost:5173`、`http://localhost:4173`、`http://127.0.0.1:5173`、`http://127.0.0.1:4173`、`https://familyday-greenworld.netlify.app`、`https://brianchang1212.github.io`。
