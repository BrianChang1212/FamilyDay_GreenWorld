...
| 2 | **遊戲說明** | 標題「遊戲說明」；**闖關說明**（**任選順序**完成 6 大關可領闖關禮）；**關卡地點列表**（線框示例：天鵝湖 → 大探奇區 → …；為園區導覽／地圖用，**不**代表必須依序遊玩；實際以場勘為準）；**注意事項**（例：1 員編最多登入 3 台裝置、最多領 3 份獎品）；主按鈕「開始探索」→ **下一屏即闖關登入頁**（不先進地圖或題目）。 |
...
| 順序 | 畫面 | 重點 |
|:----:|------|------|
| 1 | **到站／關卡頁** | 地圖可**點選**欲挑戰之站（未完成者）；顯示該站名稱；引導抵達後掃描站點 QR；**相機**開啟掃描介面。 |
| 2 | **掃描 QR** | 取景框與提示；**掃描成功**後**直接進入題目頁**（不另繞路）。 |
| 3 | **題目頁** | 闖關進度以**已完成站數／6**（非線性站號）；題幹與選項；**未選擇答案前「確定」維持不可按**；選取後可送出。 |
| 4a | **答錯** | 錯誤提示與鼓勵文案；**重新回答**回到**同一題**（或同一關題目狀態）。 |
| 4b | **答對** | 成功提示；**返回地圖**選下一個未完成之站（順序不拘）；集滿 6 站後進入完成／領獎流程。 |
...
| 2026-04-16 | **完成頁 `/finish` 領獎流程（原型）** | 對齊主規則表之次數／份數上限與櫃台驗證情境；先以瀏覽器暫存類比領取次數 | 見 `summary-frontend.md` v1.2（**現況**見 **v1.31**、`api-v0.1` **v0.1.21**）；正式上線需接 `api-v0.1.md` 櫃台／核銷端點或等價流程 |
