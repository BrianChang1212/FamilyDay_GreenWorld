...
**最後整理：** 2026-04-20。歷史變更與實作細節請以**各檔案標頭**、根目錄 **`README.md`** 頁尾版本列、以及 **`project/專案文件.md`**（合併版 **v1.3.26**）為準；**測試 Web UI** 見根 [`README.md`](../README.md#preview-netlify-test-ui)；**Vitest** 單元測試見上表與 `summary-frontend.md` **§1.1**（**v1.23**）；前端路由、`VITE_API_BASE`、`local-fallback`、`constants`／`i18n` 集中化、**`views/`** 分群與 **`api/`／`composables` 分層**見 `architecture/summary-frontend.md`（§2.1、§2，**v1.25**）；後端／流量摘要見 `architecture/summary-backend.md`（**v1.2**：**`GET /api/v1/me/dashboard`**、**`POST /api/v1/me/playthrough/restart`**）、`architecture/summary-traffic.md`（**v1.1**）；部署摘要見 `architecture/summary-deployment.md` §1.1（修訂至 **v1.4**）；`specs/api-v0.1.md` 修訂至 **v0.1.4**（**v0.1.3** Vitest 註記；**v0.1.4** 檔首 dashboard 路徑用語與端點表一致）。
