...
## 後端 MVP 落地狀態（2026-04-30）

以下為 `familyday-backend/`（`src/index.ts` 掛載 Express）目前已落地的最小端點（Firebase Functions emulator 已驗證）：
...
- CORS allowlist 已收斂：**執行時**以倉庫根 [`fdgw.project.json`](../../fdgw.project.json) 的 **`corsOrigins`** 為準（`familyday-backend` 於載入時讀取）；下列為常見值，與該陣列一致：`http://localhost:5173`、`http://localhost:4173`、`http://127.0.0.1:5173`、`http://127.0.0.1:4173`、`https://familyday-greenworld.netlify.app`、`https://brianchang1212.github.io`。
...
| v0.1.22 | 2026-05-11 | 與 `familyday-api-contract/api-v0.1.md` **v0.1.22** 同步：Mock／MVP 路徑與 CORS 載入敘述對齊 monorepo；**無** REST 或 JSON 契約變更 |
