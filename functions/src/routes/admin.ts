...
adminRouter.get("/admin/reports/attendance", (_req, res) => {
	const anyCheckin = getCheckin();
	res.status(200).json({
		total: 1000,
...
adminRouter.get("/admin/reports/progress", (_req, res) => {
	const redeem = getRedeemSummary();
	res.status(200).json({
		players: 1,
		fullClear: 0,
