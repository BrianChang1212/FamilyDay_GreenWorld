...
| 測試 | 前端 Vitest **17** 檔／**119** 測試通過（加入 `qrPayload`／`scanEntry`／`/scan` redirect／`resultAction` 四組）；後端 Vitest **5** 檔／**28** 測試通過（加入「全破後重玩不清紀錄」回歸守門）；`verify:firestore` 正式端點 **PASS**（2026-05-19）；**報到（check-in）＋闖關（game）功能於正式環境手動驗證 Pass**（2026-05-19、ResultView 三向分流 + 領獎入口手動驗證 2026-05-20） |
...
*README v2.82 · 2026-05-20（ResultView 三向分流；StageView 6/6 領獎入口 CTA；commit `08d60ee`；前版 v2.81）*
