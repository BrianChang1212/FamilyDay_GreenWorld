...
*README v2.91 · 2026-05-25（外部 QR 未登入彈窗——`/scan` redirect 不再分流到 `/register`，QuizView 在前景 mount `StageScanLoginModal` 強制登入；無取消鍵、無 backdrop dismiss；commit `fa982ba`；正式環境部署完成並驗證；前版 v2.90）*
