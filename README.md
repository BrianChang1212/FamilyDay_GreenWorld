...
| 部署 | **正式 `familyday-greenworld.web.app` 為現行主環境**（Functions `api-jwvq2npioq-uc.a.run.app`）；2026-05-26 已將 `dev` 全部變更部署並驗證、app 集合歸零（roster 508 保留）；測試環境 `rare-lattice-495009-i9.web.app`（`api-hxe6k6ncza`）保留；**正式上線仍須完成 Security Rules（`firestore.rules` 仍 deny-all）／安全基線與 1,300 人壓測** |
...
*README v2.93 · 2026-05-26（站 1 名稱對齊「天鵝湖」`7a0fc66`；相機掃碼登入連續性 `localStorage` token + `/auth/me` 回填 `17eeedb`；正式 app 集合歸零；stage 1 題目改《天鵝湖》作曲家題、正解 C `384c78c`；測試 19/131 + 5/29；前版 v2.92）*
