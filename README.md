...
| 測試 | 前端 Vitest **18** 檔／**127** 測試通過（含 `qrPayload`／`scanEntry`／`/scan`／`/reward` redirect／`resultAction`／`claimPayload`）；後端 Vitest **5** 檔／**28** 測試通過；`verify:firestore` 正式端點 **PASS**（2026-05-19）；**報到＋闖關＋ResultView 三向分流＋領獎入口＋FinishView 防誤領掃 QR 全部於正式環境手動驗證 Pass**（2026-05-19、2026-05-20） |
...
*README v2.88 · 2026-05-21（PlayerProgress 加 `rewardRedeemAt` 時間戳記陣列；commit `b8e0822`；前版 v2.87）*
