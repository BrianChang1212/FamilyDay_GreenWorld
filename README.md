...
| 測試 | 前端 Vitest **19** 檔／**133** 測試通過（含 `qrPayload`／`scanEntry`／`/scan`／`/reward` redirect／`resultAction`／`claimPayload`／`authMe`／`rewardClaimStatus.allCompleted`／`gameFlow.justFullCleared`）；後端 Vitest **5** 檔／**30** 測試通過（含 `justFullCleared` 語意）；`verify:firestore` 正式端點 **PASS**（2026-05-19）；**報到＋闖關＋ResultView 三向分流＋領獎入口＋FinishView 防誤領掃 QR 全部於正式環境手動驗證 Pass**（2026-05-19、2026-05-20） |
...
*README v3.02 · 2026-06-03(bugfix:全破後重掃 QR 重玩,結果頁誤顯示領獎鈕 — 後端新增權威 `justFullCleared`,前端改用之、不再依賴本地 sessionStorage;契約加性變更 `api-v0.1` v0.1.27;部署 functions:api+apiLoadtest+hosting;測試前端 133/後端 30;`project-master` v1.3.82;前版 v3.01)*
