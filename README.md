...
| 測試 | 前端 Vitest **19** 檔／**132** 測試通過（含 `qrPayload`／`scanEntry`／`/scan`／`/reward` redirect／`resultAction`／`claimPayload`／`authMe`／`rewardClaimStatus.allCompleted`）；後端 Vitest **5** 檔／**29** 測試通過；`verify:firestore` 正式端點 **PASS**（2026-05-19）；**報到＋闖關＋ResultView 三向分流＋領獎入口＋FinishView 防誤領掃 QR 全部於正式環境手動驗證 Pass**（2026-05-19、2026-05-20） |
...
*README v3.01 · 2026-06-02(設計稿 v9 後續批次 image copy 30–35:闖關登入頁 placeholder、StageView 圖檔掃碼按鈕+重排、stage 2/4 題庫、領獎頁文案精簡/達上限/未通關「闖關尚未完成」狀態;6 筆 hosting-only 部署 commit `6a04aed`→`75ec38a`;前端測試 131→132;`project-master` v1.3.81;前版 v3.00)*
